...
int main(void)
{
    ...
    if (feature(FEATURE_SONAR))               // Set Sonar PWM Channels depending on Rc configuration. I2C Sonar needs no further attention here
    {
        ...
        case 0:
        case 3:
            if (NumberOfMotors < 5 && !feature(FEATURE_SERVO_TILT)) pwm_params.usePWM56 = true; // Only do this with 4 Motors or less, and no Gimbal function
            break;
        case 1:
        case 4:
    ...
    if (!feature(FEATURE_PASS))
    {
        ...
        if (feature(FEATURE_SONAR))        // Initialize Sonars here depending on Rc configuration.
        {
            ...
            case 0:
            case 3:
                if (NumberOfMotors < 5 && !feature(FEATURE_SERVO_TILT)) Sonar_init();
                break;
            case 1:
            case 4:
    ...
    // Initialize some variables here before the "real thing"
    if(cfg.stat_clear == 1) ClearStats();
