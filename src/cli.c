...
const char * const mixerNames[] =
{
    "TRI", "QUADP", "QUADX", "BI",
    "GIMBAL", "Y6", "HEX6",
    "FLYING_WING", "Y4", "HEX6X", "SPIDER6X", "OCTOX8", "OCTOFLATP", "OCTOFLATX",
...
const char * const featureNames[] =
{
    ...
    "FAILSAFE", "SONAR", "PASS", "POWERMETER", "LCD",
    NULL
};
...
const clicmd_t cmdTable[] =
{
    ...
    { "flash", "enter flashmode", cliFlash },
...
const clivalue_t valueTable[] =
{
    ...
    { "gyro_cmpf_factor",          VAR_UINT16, &cfg.gyro_cmpf_factor,           10,       1000, 1 },
    { "gyro_cmpfm_factor",         VAR_UINT16, &cfg.gyro_cmpfm_factor,          10,       2000, 1 },
    ...
    { "mag_oldcalib",              VAR_UINT8,  &cfg.mag_oldcalib,                0,          1, 1 },
    { "mag_oldctime",              VAR_UINT8,  &cfg.mag_oldctime,                1,          5, 1 },
    ...
    { "mainpidctrl",               VAR_UINT8,  &cfg.mainpidctrl,                 1,          2, 1 },
...
static void cliAux(char *cmdline)
{
    ...
    else
    {
        ...
        {
            cliErrorMessage();   // printf("Invalid Feature index: must be < %u\r\n", CHECKBOXITEMS);
        }
    }
}
...
static void cliAuxset(char *cmdline)
{
    uint8_t  i, k, len, ItemID, AuxChNr;
    uint32_t val = 1;
    bool     remove = false;
    char     *ptr;
    char     buf[4];
    ...
    if (len == 0 || len < 5)
    {
        ...
        printf("ID|AUXCHAN :");
        for (i = 0; i < cfg.auxChannels; i++) printf(" %02u  ", i+1);
        printf("\r\n");
    ...
    else
    {
        ...
        if (ItemID < CHECKBOXITEMS)
        {
            ptr = strchr(ptr, ' ');
            AuxChNr = atoi(ptr);
            if (AuxChNr > cfg.auxChannels || AuxChNr == 0)
            {
                cliErrorMessage();
                return;
            }
...
static void cliStatus(char *cmdline)
{
    ...
    if (sensors(SENSOR_BARO) || sensors(SENSOR_GPS))
    {
        ...
        {
            KMH = (uint16_t)((float)cfg.MAXGPSspeed * 0.036f);
...
#define LCDdelay 12
    ...
    while (exit == 0)
    {
        ...
        if ((int32_t)(timetmp - rctimer) >= 0)                          // Start of 50Hz Loop
        {
            ...
            if (rcData[THROTTLE] < cfg.mincheck && rcData[PITCH] > cfg.maxcheck)
            {
                if (rcData[YAW] > cfg.maxcheck) exit = 1;               // Quit don't save
                if (rcData[YAW] < cfg.mincheck) exit = 2;               // Quit and save
            }
            ...
            if (exit == 0)
            {
                if (rcData[PITCH] < cfg.mincheck) input = 1;
                if (rcData[PITCH] > cfg.maxcheck) input = 2;
                if (rcData[ROLL]  < cfg.mincheck) input = 3;
                if (rcData[ROLL]  > cfg.maxcheck) input = 4;
            }
...
static void cliScanbus(char *cmdline)
{
    ...
    uartPrint("\r\n");
    if (nDevices == 0) printf("No I2C devices\r\n");
