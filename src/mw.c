...
// althold is engage autonomously after for ex: 2000ms when throttle stick is not moving deadband fix = 30 
	...
	else
	{
        ...
        {
            if ((rcOptions[BOXBARO] && GroundAltInitialized && f.ARMED) || (f.BARO_MODE && !Altholdsupp && f.ARMED && rcData[THROTTLE] < ESCnoFlyThrottle) || Altholsuppd ) //Throttle must be over esc_nfly = 1300 to go out baromode
            {
