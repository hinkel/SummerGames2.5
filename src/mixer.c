...
/*static const motorMixer_t mixerHex6P[] =                      // Old version
{
...
*/
static const motorMixer_t mixerHex6P[] =                         // Actual Good version
{ 
...
void mixTable(void)
{
    ...
    case MULTITYPE_GIMBAL:
        servo[0] = constrain(cfg.gimbal_pitch_mid + (float)cfg.gimbal_pitch_gain * angle[PITCH] * 0.0625f + rcCommand[PITCH], cfg.gimbal_pitch_min, cfg.gimbal_pitch_max);
        servo[1] = constrain(cfg.gimbal_roll_mid  + (float)cfg.gimbal_roll_gain  * angle[ROLL]  * 0.0625f + rcCommand[ROLL] , cfg.gimbal_roll_min,  cfg.gimbal_roll_max);
        break;
    ...
    if (feature(FEATURE_SERVO_TILT))
    {
        ...
        if (rcOptions[BOXCAMSTAB])
        {
            ...
            {
                servo[0] -= ((float)(-cfg.gimbal_pitch_gain) * angle[PITCH] * 0.0625f) - (float)cfg.gimbal_roll_gain * angle[ROLL] * 0.0625f;
                servo[1] += ((float)(-cfg.gimbal_pitch_gain) * angle[PITCH] * 0.0625f) + (float)cfg.gimbal_roll_gain * angle[ROLL] * 0.0625f;
            }
            else
            {
                servo[0] += (float)cfg.gimbal_pitch_gain * angle[PITCH] * 0.0625f;
                servo[1] += (float)cfg.gimbal_roll_gain  * angle[ROLL]  * 0.0625f;
            }
        }
