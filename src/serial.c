...
static void evaluateCommand(void)
{
    ...
    case MSP_SET_BOX:
        ...
        {
            tmpu32 = 0;
            if (cfg.auxChannels > 4) tmpu32 = read32();
            else tmpu32 = read16();
    ...
    case MSP_COMP_GPS:
        ...
        serialize32(GPS_time);       //UTC time of coord calc - haydent -- where should it go ? (no room in MSP_RAW_GPS) // Shikra osd time
        break;
    case MSP_ATTITUDE:
        headSerialReply(8);
        for (i = 0; i < 2; i++)
            serialize16((int16_t)angle[i]);
    ...
    case MSP_BOX:
        ...
        for (i = 0; i < CHECKBOXITEMS; i++)
        {
            ...
            if (cfg.auxChannels > 4) serialize32(tmpu32);
...
void serialCom(void)
{
    ...
    while (uartAvailable())
    {
        ...
        if (Currentprotocol == PROTOCOL_AUTOSENSE)                                  // Check for extra Data when no protocol active (that is only in disarmed state possible)
        {
            ...
            {
                NumberSignCNT++;
                if (NumberSignCNT >= 3) cliProcess();                               // This is a one way - reset - street
            }else NumberSignCNT = 0;

            if (c == 'R')
            {
                RcharCNT++;
                if (RcharCNT >= 3) systemReset(true);                               // This is a one way - reset - street
            }else RcharCNT = 0;
        }
